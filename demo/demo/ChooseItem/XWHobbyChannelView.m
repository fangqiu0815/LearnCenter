...
#import "XWHobbyView.h"

@interface XWHobbyChannelView()
...
/** 移动前后位置 model*/
@property (nonatomic, strong) XWHobbyItemModel *oldModel;
/** 当前频道所在的view */
@property (nonatomic, strong) XWHobbyView *initialView;
/** 旧位置 */
...
@implementation XWHobbyChannelView
...
-(void)initialize {
    ...
    coverImage = MyImage(@"");
    placeHolderImage = [UIImage imageNamed:@"channel_bg"];
    edgeX = 15.f;
    spaceX = 10.0;
    spaceY = 0.0;
    topY = 50.0;
    numberOfLine = 4;
    channelW = 80*AdaptiveScale_W;
    channelH = 50*AdaptiveScale_W;
    isLongTapScale = YES;
...
+(instancetype)channelViewWithFrame:(CGRect)frame Manager:(XWHobbyItemManager *)manager{
    ...
    XWHobbyChannelView *channelView = [[XWHobbyChannelView alloc]initWithFrame:frame];
    
    NSAssert(manager, @"XWHobbyItemManager不得为空");
...
-(void)initializeSubView{
    ...
    [self.editBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.topTipView);
...
-(void)configTopChannelView{
    ...
    for (XWHobbyItemModel *model in self.manager.topChannelArr) {
        
        XWHobbyView *view = [[XWHobbyView alloc]initWithFrame:CGRectMake(edgeX + idx%numberOfLine*(channelW+spaceX), topY + (idx/numberOfLine)*(channelH+spaceY), channelW, channelH)];
        view.model = model;
...
-(void)configBottomChannelView{
    ...
    for (XWHobbyItemModel *model in self.manager.bottomChannelArr) {
        
        XWHobbyView *view = [[XWHobbyView alloc]initWithFrame:CGRectMake(edgeX + idx%numberOfLine*(channelW+spaceX), (idx/numberOfLine)*(channelH+spaceY) + bottomY, channelW, channelH)];
        view.model = model;
        view.isEdit = NO;
    ...
    if (self.bottomViewArr.count>0) {
        XWHobbyView *bottomlastView = self.bottomViewArr[self.bottomViewArr.count - 1];
...
-(void)configBottomTipView{
    XWHobbyView *toplastView = self.topViewArr[self.topViewArr.count - 1];
    CGFloat bottomY = toplastView.frame.origin.y + toplastView.bounds.size.height;
...
-(void)reconfigTopViewsWithAnimation:(BOOL)animation{
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        [self.topViewArr enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[XWHobbyView class]]) {
                XWHobbyView *channelView = (XWHobbyView *)obj;
...
-(void)setIsEdit:(BOOL)isEdit{
    ...
    if (isEdit) {
        ...
        [self.topViewArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([obj isKindOfClass:[XWHobbyView class]]) {
                XWHobbyView *view = (XWHobbyView *)obj;
                view.contentLabel.textColor = self.manager.style.normalTextColor;
                view.isEdit = YES;
                //为了避免平移手势与scrollView滑动手势冲突
                XWHobbyItemModel *model = self.manager.topChannelArr[idx];
                if (!model.isEnable) {
                    view.panGes = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(toppanGesture:)];
                }
            }
...
-(void)toppanGesture:(UIPanGestureRecognizer *)panGes{
    ...
    XWHobbyView *touchView = (XWHobbyView *)panGes.view;
    ...
    if (panGes.state == UIGestureRecognizerStateBegan) {
        ...
        if (_currentIndex > self.topViewArr.count - 1) {
            ...
            XWHobbyItemModel *currentModel = self.manager.topChannelArr[_currentIndex];
...
-(void)toplongGesture:(UILongPressGestureRecognizer *)longGes{
    ...
    XWHobbyView *touchView = (XWHobbyView *)longGes.view;
    [touchView.superview bringSubviewToFront:touchView];
    CGPoint movePoint;
...
-(void)toptapGesture:(UITapGestureRecognizer *)tapGes{
    ...
    XWHobbyView *topView = (XWHobbyView *)tapGes.view;
    ...
    if (!self.isEdit) {
        ...
        XWHobbyItemModel *currentModel = self.manager.topChannelArr[currentIndex];
        if (currentModel.isEnable) {
            return;
        }
...
-(void)bottomtapGesture:(UITapGestureRecognizer *)tapGes{
    ...
    XWHobbyView *bottomView = (XWHobbyView *)tapGes.view;
    NSUInteger currentIndex = [self.bottomViewArr indexOfObject:bottomView];
    XWHobbyItemModel *currentModel = self.manager.bottomChannelArr[currentIndex];
...
-(UIView *)topTipView{
    ...
    if (!_topTipView) {
        ...
        label.frame = CGRectMake(15, 0, _topTipView.bounds.size.width-edgeX-self.manager.style.padding, _topTipView.bounds.size.height);
...
-(UIView *)bottomTipView{
    if (!_bottomTipView) {
        ...
        label.text = @"推荐频道";
        label.font = [UIFont systemFontOfSize:15.0];
        label.frame = CGRectMake(15, 0, _topTipView.bounds.size.width-edgeX-self.manager.style.padding, topY);
