...
@implementation UIImageView (YLNoImageModel)
    ...
    if(kYLNoImageModeIsAvaliable){
        ...
        [[SDWebImageManager sharedManager]cachedImageExistsForURL:url completion:^(BOOL isInCache) {
            if(isInCache){
                ...
                [self sd_setImageWithURL:url placeholderImage:holderImg];
            }else{
                //如果没有加载过，则显示点击查看显示图片
                self.image = holderImg;
                ...
                if(!self.coverButton){
                    ...
                    self.coverButton.titleLabel.font = [UIFont systemFontOfSize:12];
                    [self.coverButton setTitleColor:MainGrayTextColor forState:UIControlStateNormal];
                    [self.coverButton setBackgroundColor:MainBGColor];
...
-(void)coverButtonAction
{
    ...
    [self sd_setImageWithURL:self.loadImageUrl placeholderImage:self.holderImage];
}
