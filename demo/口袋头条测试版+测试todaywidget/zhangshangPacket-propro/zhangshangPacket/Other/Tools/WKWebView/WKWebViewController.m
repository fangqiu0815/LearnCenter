...
@interface WKWebViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate,UIScrollViewDelegate,XFPublishViewDelegate>
...
@property (nonatomic, copy) NSString *ingotView;
...
@property (nonatomic, strong) GCD_Timer *timer1;
...
- (void)watchNewsReturnAndId:(int)newsid
{
    [STRequest WatchADBackDataWithParam:newsid WithDataBlock:^(id ServersData, BOOL isSuccess) {
        if (isSuccess) {
            ...
            _isFinishTask = 1;
            NSDictionary *dict = @{
                                   @"isFinishTask":@(_isFinishTask)
                                   };
            [[NSNotificationCenter defaultCenter] postNotificationName:@"sendSuccessFinishTask" object:nil userInfo:dict];
                        ...
                        if (_artid < SYS_AD_MINID || _artid > SYS_AD_MAXID) {
                            if ([_newstype intValue] == 0) {//普通文章
                                ...
                                if ([readingArr count] == 0) {
                                    JLLog(@"空taskstatus列表 --- ingot---%@----status---%@",ingot,status);
                                    if ([status integerValue] == -1) {
                                        JLLog(@"空taskstatus列表 --当前类型任务已完成- ingot---%@----status---%@",ingot,status);
                                        
                                    } else if([status integerValue] == 1){
                                        JLLog(@"空taskstatus列表 --当前任务类型已完成- ingot---%@----status---%@",ingot,status);
                                        
                                    } else{
                                        JLLog(@"空taskstatus列表 --当前任务类型进行中- ingot---%@----status---%@",ingot,status);
                            ...
                            }else if([_newstype intValue] == 2){
                                //分享奖励 看文章无奖励
                                [SVProgressHUD showSuccessWithStatus:@"你阅读的文章需要分享才能获得奖励哦"];
                                
                            }else if ([_newstype intValue] == 3){
                                //当前是视频
...
//{
//
//    self.WXinstall = [[UIAlertView alloc]initWithTitle:self.ingotView message:nil delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//}
...
- (void)shareClick:(id)sender
{
    ...
    if (STUserDefaults.empudid == 1) {
        ...
        if (STUserDefaults.isJiagu == 1) {
            XWHomePublishView *publishView = [[XWHomePublishView alloc]initWithFrame:[UIScreen mainScreen].bounds];
...
//    NSString *urlStringMore = [NSString stringWithFormat:@"%@&uid=%@&name=%@&id=%d&longitude=%@&latitude=%@",_urlStr,STUserDefaults.uid,STUserDefaults.name,_artid,STUserDefaults.longitude,STUserDefaults.latitude];
    NSString *urlStringMore = [NSString stringWithFormat:@"%@",_urlStr];
    JLLog(@"url--%@",urlStringMore);
    //创建分享消息对象
    UMSocialMessageObject *messageObject = [UMSocialMessageObject messageObject];
    
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:_titleStr descr:_titleStr thumImage:[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:_imageStr]]]];
    //设置网页地址
                    ...
                    if ([data isKindOfClass:[UMSocialShareResponse class]]) {
                        ...
                        if (_artid < SYS_AD_MINID || _artid > SYS_AD_MAXID) {//文章
                            if ([_newstype intValue] == 2) {
                                //分享奖励
                                [self setupAlertViewWithTitle:[NSString stringWithFormat:@"%ld",(long)_adIngot]];
                                JLLog(@"type == 2 分享奖励");
                            }else{
...
#pragma mark ======================= 延时5s判断任务进行操作 ==================================

    if (STUserDefaults.ischeck == 1) {
        
    } else {
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            if (isFinishWatchTask == 1) {
//                
//            } else {
...
//            
//        });
        GCD_Timer *timer1 = [GCD_Timer timerWithInterval:5.0 repeats:NO target:self selector:@selector(otherOutput)];
        self.timer1 = timer1;
        [timer1 resumeTimer];
...
-(void)webView:(WKWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation{
    ...
    if ([_newstype intValue] == 3) {//看视频广告
        NSURL  *movieURL = [NSURL URLWithString:_urlStr];
        ...
        JLLog(@"播放时长(s)---%d", second);
        if (second >= 60) {
            int index = second / 60;
            minute = index;
            second = second - index*60;
            JLLog(@"播放时长(m:s)---%d:%d",minute,second);
        }
        JLLog(@"看视频广告");
        if (STUserDefaults.ischeck == 1) {
            ...
            GCD_Timer *timer1 = [GCD_Timer timerWithInterval:second+2 repeats:NO target:self selector:@selector(videoOutput)];
            self.timer1 = timer1;
            [timer1 resumeTimer];
   ...
   // [SVProgressHUD showSuccessWithStatus:@"需要读完文章才有奖励"];
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    // 打开appstore
    if ([url.absoluteString containsString:@"https://itunes.apple.com/cn/app/"] || [url.absoluteString containsString:@"http://a.app.qq.com/o/simple.jsp?pkgname"])
    {
...
//    self.numLab.text = [NSString stringWithFormat:@"＋%@元宝",titleString];
...
//    [self presentViewController:alertController animated:YES completion:nil];
    ...
    NSDictionary *dict = @{
                           @"allingot":titleString
                           };
    [[NSNotificationCenter defaultCenter] postNotificationName:@"sendIngotToMine" object:nil userInfo:dict];
...
//        Configuration.preferences = preference;
        ...
        _wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, ScreenW, ScreenH) configuration:Configuration];
