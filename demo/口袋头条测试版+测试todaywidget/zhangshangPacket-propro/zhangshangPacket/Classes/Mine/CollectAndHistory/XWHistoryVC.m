...
@interface XWHistoryVC ()<DZNEmptyDataSetDelegate,
...
- (void)topicAdd
{
    ...
    self.infoArr = [XWHomeDataTool topics];
    JLLog(@"%@",self.infoArr);
...
-(void)selectClick{
    ...
    if (self.infoArr.count == 0) {
        [SVProgressHUD showSuccessWithStatus:@"当前无阅读记录"];
    } else {
...
{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"确认删除所有历史记录?删除后无法恢复" message:nil preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction *action = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [XWHomeDataTool deleteAllDataFromSql];
