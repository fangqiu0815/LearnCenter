...
- (void)setModel:(NewsListDataModel *)model
{
    ...
    [self.imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",model.imgsUrl]] placeholderImage:MyImage(@"bg_default")];
//    self.timeLab.text = [NSString stringWithFormat:@"当前时间"];
    JLLog(@"title---%@---imgurl---%@",model.title,model.imgsUrl);
