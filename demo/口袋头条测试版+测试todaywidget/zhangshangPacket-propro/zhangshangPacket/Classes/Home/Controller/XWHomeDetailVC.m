...
@interface XWHomeDetailVC ()<XFPublishViewDelegate>
{
    NSInteger _cellType;
...
@property (nonatomic, strong) NSMutableArray *arrtemparr;

@property (nonatomic, assign) NSInteger isfinishTask;
@property (nonatomic, strong) XWTripleImageCell *tripleImgcell;

@end

@implementation XWHomeDetailVC
...
- (void)setupUI
{
    ...
    [self.tableView registerClass:[XWTripleImageCell class] forCellReuseIdentifier:@"XWTripleImageCell"];
    [self.tableView registerClass:[XWBigPicCell class] forCellReuseIdentifier:@"XWBigPicCell"];
    [self setupRefresh];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setupRefresh) name:@"RefreshXWInfo" object:nil];
...
- (void)setupRefresh
{
    ...
    if (STUserDefaults.isLogin){
        JLLog(@"isfirst");
        _artidLast = 0;
        JLLog(@"firstLoadData--artidlast---%ld",(long)_artidLast);
        [self loadNewFirstDataWithParam:_artidLast];
...
#pragma mark ========== 处理 下拉刷新 无数据的 ===========
    if (STUserDefaults.isLogin) {
        ...
        [self loadNewDataWithParam:_artidLast];
...
#pragma mark ============= 处理 上拉加载更多 无数据的 =============
        if (isFirstGet) {
            JLLog(@"artidfirstData---%ld---isfirstget--%ld",(long)_artidFirstData,(long)isFirstGet);
            [self loadOldDataWithParam:_artidFirstData];

        } else {
            [self loadOldDataWithParam:_artidFirst];
...
- (void)loadNewFirstDataWithParam:(NSInteger)artid
{
    ...
    [STRequest NewsInfoNewDataParam:artid andDireParam:direDown andDataBlock:^(id ServersData, BOOL isSuccess) {
...
#pragma  mark ============= 第一次加载数据的时候 =========
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(10.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            
                            [self setupInformView];
...
- (void)setupInformView{
    NSString *openFirst = [[NSUserDefaults standardUserDefaults] objectForKey:@"IsFirestOpen"];
    if ([openFirst isEqualToString:@"1"]) {
        
        XWPublishInformView *publishView = [[XWPublishInformView alloc]initWithFrame:[UIScreen mainScreen].bounds];
        ...
        JLLog(@"不是第一次打开 即要服务端给接口打开");
        if (STUserDefaults.boundwx == 1) {
            XWPublishInformView *publishView = [[XWPublishInformView alloc]initWithFrame:[UIScreen mainScreen].bounds];
            publishView.layer.masksToBounds = YES;
            publishView.layer.cornerRadius = 5.0;
            publishView.delegate = self;
...
- (void)loadOldDataWithParam:(NSInteger)artid
{
    ...
    isFirstGet = 0;
    [self.muarray removeAllObjects];
    [STRequest NewsInfoNewDataParam:artid andDireParam:direUP andDataBlock:^(id ServersData, BOOL isSuccess) {
        JLLog(@"ServersData----%@",ServersData);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([model.type integerValue] == 5){
        ...
        XWHomeDetailNewsVC *checkNewsVC = [[XWHomeDetailNewsVC alloc]init];
        checkNewsVC.newsId = model.id;
        checkNewsVC.newsTitle = model.title;
        checkNewsVC.listModel = model;
        [self.navigationController pushViewController:checkNewsVC animated:YES];
        
    }else{
        [XWHomeDataTool saveTopic:model];
        ...
        JLLog(@"url ---- %@",model.url);
        //广告下的 分享 ingot
        detailsController.adIngot = [model.ingot integerValue];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.showType integerValue] == 2) {
        ...
        return  [tableView fd_heightForCellWithIdentifier:@"XWTripleImageCell" cacheByIndexPath:indexPath configuration:^(XWTripleImageCell *cell) {
...
- (NSMutableArray *)arrtemparr
{
    if (!_arrtemparr) {
        _arrtemparr = [NSMutableArray array];
    }
    return _arrtemparr;
}
