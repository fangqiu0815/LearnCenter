...
- (void)setupRefresh
{
    ...
    if (STUserDefaults.isLogin){
        ...
        //[self loadNewDataWithParam:_artidFirst];
...
{
    if (STUserDefaults.isLogin) {
        JLLog(@"artidfirst---%ld---artidlast---%ld",(long)self.artidFirst,(long)self.artidLast);
...
- (void)loadNewFirstDataWithParam:(NSInteger)artid
{
    ...
    [STRequest EnterTainInfoNewDataParam:artid andDireParam:direDown andDataBlock:^(id ServersData, BOOL isSuccess) {
        ...
        if (isSuccess) {
            if ([ServersData[@"c"] intValue] == 1) {
                ...
                NSMutableArray *arrtemp = [NSMutableArray arrayWithArray:dictemp[@"disportlist"]];
        ...
        if (isSuccess) {
            if ([ServersData[@"c"] intValue] == 1) {
                ...
                if (arrtemp.count == 0) {
                    ...
                    JLLog(@"first---%ld  last---%ld",(long)_artidFirst,(long)_artidLast);
...
- (void)loadOldDataWithParam:(NSInteger)artid
{
    ...
    [STRequest EnterTainInfoNewDataParam:artid andDireParam:direUP andDataBlock:^(id ServersData, BOOL isSuccess) {
        self.dataModel = [XWHomeModel mj_objectWithKeyValues:ServersData];
        if (isSuccess) {
            [self.muarray removeAllObjects];
            if ([ServersData[@"c"] intValue] == 1) {
