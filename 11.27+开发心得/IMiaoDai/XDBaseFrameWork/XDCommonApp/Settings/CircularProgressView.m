...
//    }
    [self performSelector:@selector(aa:) withObject:@"0.2" afterDelay:0];
    
    [self performSelector:@selector(aa:) withObject:@"0.3" afterDelay:0.5];
    
    [self performSelector:@selector(aa:) withObject:@"0.6" afterDelay:0.9];
    
    [self performSelector:@selector(aa:) withObject:@"0.8" afterDelay:1.6];
    
    [self performSelector:@selector(aa:) withObject:@"1.0" afterDelay:2.8];
    
}

-(void)aa:(NSString *)str{
    self.progress = [str floatValue];
    [self setNeedsDisplay];
}
- (void)play{
